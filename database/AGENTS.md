...
Row-level security is enabled on the tables created by the migrations. Treat policy and grants changes as security-sensitive. `views.sql` defines `latest_temperature_readings` and grants it to `anon`, `authenticated`, and `service_role`.
